const express = require('express');
const app = express();
const PORT = process.env.PORT || 3000;

app.use(express.json());

app.get('/', (req, res) => {
  res.send('Welcome to the Car Products API!');
});

let products = [
  {
    id : 1,
    image : "data:image/png;base64,",
    carname : "Ram Van B250",
    year : 1993,
    manufacturer : "Dodge",
    vin : "JN1AZ4EH9EM793379"
  },
  {
    id : 2,
    image : "data:image/png;base64,",
    carname : "Caravan",
    year : 2005,
    manufacturer : "Dodge",
    vin : "3VWKX7AJ3CM671357"
  },
  {
    id : 3,
    image : "data:image/png;base64,",
    carname : "Grand Prix",
    year : 1999,
    manufacturer : "Pontiac",
    vin : "2G4GL5EX0E9001943"
  },
  {
    id : 4,
    image : "data:image/png;base64,",
    carname : "Quattroporte",
    year : 2010,
    manufacturer : "Maserati",
    vin : "JTDZN3EU5E3069519"
  },
  {
    id : 5,
    image : "data:image/png;base64,",
    carname : "QX",
    year : 2002,
    manufacturer : "Infiniti",
    vin : "JM3ER2A57B0485958"
  },
  {
    id : 6,
    image : "data:image/png;base64,",
    carname : "Optima",
    year : 2011,
    manufacturer : "Kia",
    vin : "1D7RB1CT6AS632820"
  },
  {
    id : 7,
    image : "data:image/png;base64,",
    carname : "Clubman",
    year : 2009,
    manufacturer : "MINI",
    vin : "3FADP4AJ9CM597135"
  },
  {
    id : 8,
    image : "data:image/png;base64,",
    carname : "G",
    year : 2012,
    manufacturer : "Infiniti",
    vin : "WAUBF98EX8A722722"
  },
  {
    id : 9,
    image : "data:image/png;base64,",
    carname : "Expedition",
    year : 2008,
    manufacturer : "Ford",
    vin : "1FTEX1C85AF335677"
  },
  {
    id : 10,
    image : "data:image/png;base64,",
    carname : "Firebird",
    year : 1999,
    manufacturer : "Pontiac",
    vin : "WBA3A9C52EF247589"
  },
  {
    id : 11,
    image : "data:image/png;base64,",
    carname : "Explorer",
    year : 2000,
    manufacturer : "Ford",
    vin : "3FAHP0CG6BR508554"
  },
  {
    id : 12,
    image : "data:image/png;base64,",
    carname : "Camaro",
    year : 1969,
    manufacturer : "Chevrolet",
    vin : "WAUDF58E35A666958"
  },
  {
    id : 13,
    image : "data:image/png;base64,",
    carname : "Diamante",
    year : 1998,
    manufacturer : "Mitsubishi",
    vin : "1G6DP567250581641"
  },
  {
    id : 14,
    image : "data:image/png;base64,",
    carname : "Cougar",
    year : 1969,
    manufacturer : "Mercury",
    vin : "5N1AT2MK2FC135227"
  },
  {
    id : 15,
    image : "data:image/png;base64,",
    carname : "Truck",
    year : 1992,
    manufacturer : "Mitsubishi",
    vin : "5N1AT2ML1FC134370"
  },
  {
    id : 16,
    image : "data:image/png;base64,",
    carname : "I",
    year : 2002,
    manufacturer : "Infiniti",
    vin : "2G4WD582691687550"
  },
  {
    id : 17,
    image : "data:image/png;base64,",
    carname : "ZDX",
    year : 2011,
    manufacturer : "Acura",
    vin : "4USFE83517L653683"
  },
  {
    id : 18,
    image : "data:image/png;base64,",
    carname : "V70",
    year : 2009,
    manufacturer : "Volvo",
    vin : "5TDBY5G1XES816697"
  },
  {
    id : 19,
    image : "data:image/png;base64,",
    carname : "Viper",
    year : 2001,
    manufacturer : "Dodge",
    vin : "YV4940BZ4D1772246"
  },
  {
    id : 20,
    image : "data:image/png;base64,",
    carname : "S2000",
    year : 2002,
    manufacturer : "Honda",
    vin : "WAULC68E55A465282"
  },
  {
    id : 21,
    image : "data:image/png;base64,",
    carname : "H1",
    year : 2006,
    manufacturer : "Hummer",
    vin : "WBA3F9C53EF543304"
  },
  {
    id : 22,
    image : "data:image/png;base64,",
    carname : "Maxima",
    year : 1997,
    manufacturer : "Nissan",
    vin : "WAUXG74F99N772317"
  },
  {
    id : 23,
    image : "data:image/png;base64,",
    carname : "Ram 3500",
    year : 2004,
    manufacturer : "Dodge",
    vin : "3D73M4EL3AG842975"
  },
  {
    id : 24,
    image : "data:image/png;base64,",
    carname : "Grand Marquis",
    year : 1996,
    manufacturer : "Mercury",
    vin : "JN8AZ2KR1DT741252"
  },
  {
    id : 25,
    image : "data:image/png;base64,",
    carname : "ES",
    year : 2007,
    manufacturer : "Lexus",
    vin : "1G6DM5E39D0725692"
  },
  {
    id : 26,
    image : "data:image/png;base64,",
    carname : "Explorer Sport Trac",
    year : 2002,
    manufacturer : "Ford",
    vin : "5XYKT3A12CG850073"
  },
  {
    id : 27,
    image : "data:image/png;base64,",
    carname : "T100",
    year : 1995,
    manufacturer : "Toyota",
    vin : "1G6AB5SAXD0373122"
  },
  {
    id : 28,
    image : "data:image/png;base64,",
    carname : "Quest",
    year : 1997,
    manufacturer : "Nissan",
    vin : "WBABD33416J216162"
  },
  {
    id : 29,
    image : "data:image/png;base64,",
    carname : "V70",
    year : 2006,
    manufacturer : "Volvo",
    vin : "5GAKVBKD2EJ647704"
  },
  {
    id : 30,
    image : "data:image/png;base64,",
    carname : "SRX",
    year : 2011,
    manufacturer : "Cadillac",
    vin : "1G6DG577890601802"
  },
  {
    id : 31,
    image : "data:image/png;base64,",
    carname : "Legacy",
    year : 2000,
    manufacturer : "Subaru",
    vin : "4T1BF3EK8BU263329"
  },
  {
    id : 32,
    image : "data:image/png;base64,",
    carname : "Azera",
    year : 2011,
    manufacturer : "Hyundai",
    vin : "WAUWMAFC7FN338297"
  },
  {
    id : 33,
    image : "data:image/png;base64,",
    carname : "Patriot",
    year : 2011,
    manufacturer : "Jeep",
    vin : "19VDE1F55FE108408"
  },
  {
    id : 34,
    image : "data:image/png;base64,",
    carname : "Continental",
    year : 1987,
    manufacturer : "Lincoln",
    vin : "1G4GE5GV3AF532965"
  },
  {
    id : 35,
    image : "data:image/png;base64,",
    carname : "Mazda6",
    year : 2009,
    manufacturer : "Mazda",
    vin : "KNADM4A3XD6710318"
  },
  {
    id : 36,
    image : "data:image/png;base64,",
    carname : "Liberty",
    year : 2006,
    manufacturer : "Jeep",
    vin : "1G6DN57SX40488020"
  },
  {
    id : 37,
    image : "data:image/png;base64,",
    carname : "Verona",
    year : 2005,
    manufacturer : "Suzuki",
    vin : "SCFFDCBD0AG126434"
  },
  {
    id : 38,
    image : "data:image/png;base64,",
    carname : "2500",
    year : 1997,
    manufacturer : "GMC",
    vin : "SAJWA4GB2EL710658"
  },
  {
    id : 39,
    image : "data:image/png;base64,",
    carname : "QX",
    year : 1997,
    manufacturer : "Infiniti",
    vin : "3D7TT2CT6AG937508"
  },
  {
    id : 40,
    image : "data:image/png;base64,",
    carname : "Corolla",
    year : 1992,
    manufacturer : "Toyota",
    vin : "3N1CN7AP6DL170684"
  },
  {
    id : 41,
    image : "data:image/png;base64,",
    carname : "Suburban 1500",
    year : 2003,
    manufacturer : "Chevrolet",
    vin : "SCFFDAAE8AG247386"
  },
  {
    id : 42,
    image : "data:image/png;base64,",
    carname : "LR2",
    year : 2011,
    manufacturer : "Land Rover",
    vin : "WBAUP9C56AV832744"
  },
  {
    id : 43,
    image : "data:image/png;base64,",
    carname : "530",
    year : 2005,
    manufacturer : "BMW",
    vin : "1N6AA0CA9BN094422"
  },
  {
    id : 44,
    image : "data:image/png;base64,",
    carname : "Firebird",
    year : 1999,
    manufacturer : "Pontiac",
    vin : "1G6KD579X6U948201"
  },
  {
    id : 45,
    image : "data:image/png;base64,",
    carname : "Leganza",
    year : 2000,
    manufacturer : "Daewoo",
    vin : "1FTSX2A56AE961620"
  },
  {
    id : 46,
    image : "data:image/png;base64,",
    carname : "Charger",
    year : 1968,
    manufacturer : "Dodge",
    vin : "1G4PR5SK5E4364667"
  },
  {
    id : 47,
    image : "data:image/png;base64,",
    carname : "LHS",
    year : 2001,
    manufacturer : "Chrysler",
    vin : "WBAXH5C5XCC851914"
  },
  {
    id : 48,
    image : "data:image/png;base64,",
    carname : "Safari",
    year : 2004,
    manufacturer : "GMC",
    vin : "WBANU53519C753754"
  },
  {
    id : 49,
    image : "data:image/png;base64,",
    carname : "Astro",
    year : 1994,
    manufacturer : "Chevrolet",
    vin : "3N1BC1AP7AL961526"
  },
  {
    id : 50,
    image : "data:image/png;base64,",
    carname : "E150",
    year : 2012,
    manufacturer : "Ford",
    vin : "4T1BD1FK8EU874605"
  },
];

let users = [
  {id :1, first_name:"Lyon", last_name :"Enever", email :"lenever0@flickr.com", password :"hE3?Q="},
  {id :2, first_name:"Angil", last_name :"Mountfort", email :"amountfort1@princeton.edu", password :"rF4}C/Zj"},
  {id :3, first_name:"Christi", last_name :"Bellward", email :"cbellward2@bloglovin.com", password :"nL4#g"},
  {id :4, first_name:"Kristopher", last_name :"Ioselev", email :"kioselev3@creativecommons.org", password :"dL1)O?"},
  {id :5, first_name:"Haslett", last_name :"Feaster", email :"hfeaster4@columbia.edu", password :"pB2`"},
  {id :6, first_name:"Carissa", last_name :"Mosdall", email :"cmosdall5@google.nl", password :"qG5,"},
  {id :7, first_name:"Dieter", last_name :"Ellacott", email :"dellacott6@about.com", password :"bV8<iJ"},
  {id :8, first_name:"Aguistin", last_name :"Featherstonhaugh", email :"afeatherstonhaugh7@google.cn", password :"xE1`Q"},
  {id :9, first_name:"Rockwell", last_name :"Woof", email :"rwoof8@ft.com", password :"gD2~"},
  {id :10, first_name:"Caro", last_name :"Dye", email :"cdye9@artisteer.com", password :"tK0@|nx"},
  {id :11, first_name:"Zia", last_name :"Grierson", email :"zgriersona@lycos.com", password :"yP0="},
  {id :12, first_name:"Sonia", last_name :"Flude", email :"sfludeb@illinois.edu", password :"hE5|~aYb"},
  {id :13, first_name:"Anatollo", last_name :"Stean", email :"asteanc@mediafire.com", password :"hP2_DAA"},
  {id :14, first_name:"Alasdair", last_name :"Ivashkin", email :"aivashkind@ed.gov", password :"vW2#KC"},
  {id :15, first_name:"Egor", last_name :"Gorch", email :"egorche@cbsnews.com", password :"lA7!k'e,"},
  {id :16, first_name:"Penny", last_name :"Beden", email :"pbedenf@theglobeandmail.com", password :"wY6'}6E"},
  {id :17, first_name:"Murielle", last_name :"Kordt", email :"mkordtg@who.int", password :"nS2.DW"},
  {id :18, first_name:"Hatty", last_name :"Revelle", email :"hrevelleh@constantcontact.com", password :"tD6_)u"},
  {id :19, first_name:"Filmer", last_name :"Rawlings", email :"frawlingsi@mac.com", password :"dQ4\\`"},
  {id :20, first_name:"Melba", last_name :"Timmins", email :"mtimminsj@sciencedirect.com", password :"vH3,Q"}
];

app.get('/products', (req, res) => {
  const { manufacturer, year, carname } = req.query;
  let filteredProducts = products;

  if (manufacturer) {
    filteredProducts = filteredProducts.filter(p => p.manufacturer.toLowerCase() === manufacturer.toLowerCase());
  }
  if (year) {
    filteredProducts = filteredProducts.filter(p => p.year === parseInt(year, 10));
  }
  if (carname) { 
    filteredProducts = filteredProducts.filter(p => p.carname.toLowerCase() === carname.toLowerCase());
  }

  res.json(filteredProducts);
});

app.get('/products/:id', (req, res) => {
  const product = products.find(p => p.id === parseInt(req.params.id));
  if (!product) return res.status(404).send('Product not found.');
  res.json(product);
});

app.post('/products', (req, res) => {
  const { carname, manufacturer, year, image, vin } = req.body; 

  if (!carname || !manufacturer || !year || !image || !vin) {
    return res.status(400).json({ message: 'All fields are required' });
  }

  const newProduct = {
    id: products.length + 1,
    carname,
    manufacturer,
    year,
    image,
    vin
  };

  products.push(newProduct);
  res.status(201).json(newProduct);
});

app.put('/products/:id', (req, res) => {
  const product = products.find(p => p.id === parseInt(req.params.id));
  if (!product) return res.status(404).send('Product not found.');

  product.carname = req.body.carname || product.carname; 
  product.manufacturer = req.body.manufacturer || product.manufacturer;
  product.year = req.body.year || product.year;
  product.image = req.body.image || product.image; 
  product.vin = req.body.vin || product.vin; 

  res.json(product);
});

app.delete('/products/:id', (req, res) => {
  const productId = parseInt(req.params.id);
  const productIndex = products.findIndex(product => product.id === productId);
  
  if (productIndex !== -1) {
    products.splice(productIndex, 1); // Remove the product
    res.json({ message: 'Product deleted successfully.' });
  } else {
    res.status(404).json({ message: 'Product not found.' });
  }
});

app.get('/users', (req, res) => {
  res.json(users);
});

app.get('/users/:id', (req, res) => {
  const user = users.find(u => u.id === parseInt(req.params.id));
  if (!user) return res.status(404).send('User not found.');
  res.json(user);
});

app.post('/users', (req, res) => {
  const { first_name, last_name, email, password } = req.body;

  if (!first_name || !last_name || !email || !password) {
    return res.status(400).json({ message: 'All fields are required' });
  }

  const existingUser = users.find(u => u.email === email);
  if (existingUser) {
    return res.status(400).json({ message: 'Email already in use.' });
  }

  const newUser = {
    id: users.length + 1,
    first_name,
    last_name,
    email,
    password
  };

  users.push(newUser);
  res.status(201).json({ message: 'User created successfully.', user: newUser });
});

app.put('/users/:id', (req, res) => {
  const user = users.find(u => u.id === parseInt(req.params.id));
  if (!user) return res.status(404).send('User not found.');

  user.first_name = req.body.first_name || user.first_name;
  user.last_name = req.body.last_name || user.last_name; 
  user.email = req.body.email || user.email; 
  user.password = req.body.password || user.password; 

  res.json(user);
});

app.delete('/users/:id', (req, res) => {
  const userId = parseInt(req.params.id);
  const userIndex = users.findIndex(user => user.id === userId);
  
  if (userIndex !== -1) {
    users.splice(userIndex, 1);
    res.json({ message: 'User deleted successfully.' });
  } else {
    res.status(404).json({ message: 'User not found.' });
  }
});

app.listen(PORT, () => {
  console.log(`Server is running on port ${PORT}`);
});